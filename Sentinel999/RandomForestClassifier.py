...
def load_data_and_predict(input_df):
    base_dir = os.path.dirname(os.path.abspath(__file__))  # Definisce la variabile

    combined_data_path = os.path.join(base_dir, 'Combine_csv', 'combined_data.csv')
    model_path = os.path.join(base_dir, 'Models', 'Ensemble', 'random_forest_model.pkl')

    combined_df = pd.read_csv(combined_data_path)
    model = joblib.load(model_path)
    
    combined_features_encoded = pd.get_dummies(input_df[['Transporter', 'Sales_Channel']], drop_first=True)
    predicted_probs = model.predict_proba(combined_features_encoded)

    input_df['Predicted_Risk'] = predicted_probs[:, 1] if predicted_probs.shape[1] == 2 else 0
    input_df['Risk_Category'] = pd.cut(input_df['Predicted_Risk'], bins=[0.1, 0.2, 0.3, 0.35, 0.45, 1.0],
                                       labels=['Rischio Molto Basso', 'Rischio Basso', 'Rischio Medio', 'Rischio Alto', 'Rischio Molto Alto'])
    return input_df
...
combined_data_path = os.path.join(base_dir, 'Combine_csv', 'combined_data.csv')
counterfeit_data_path = os.path.join(base_dir, 'Combine_csv', 'prodotti_contraffatti.csv')
